...
If patient wasn't there when calling , patient care can keep that queue serial on hold and releases if that patient has come 
...
Patient New/OLD option , old patient disables writing on 
Name Text
Age Number
...
Address Text
Phone No Text
MRN No : MRN No Auto-generated ( for new patient) or manually entered (for old patient) , if the condition is old patient all patient info shall be collected from the old data based on MRN No
Department Dropdown (Cataract, Retina, Glaucoma, Cornea, Private, General) will actually show whatever department was created by the admin
Emergency Boolean (Yes/No)
Specific Doctor Dropdown (filtered by Department) or left blank for Patient Care to assign

All patient data is stored in the database, old and new both.
...
Configure each of the six departments or more:
...
# this are just dummy make improvements as required
...
This are all just dummy info, there are many missing things, so improve the design and the code snippets
